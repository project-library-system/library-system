...
export class BookPrismaRepository implements BookRepository {
    ...
    async create(data: CreateBookProps): Promise<Book> {
        ...
        return new Book(createdBook.id, createdBook.isbn, createdBook.name, createdBook.author, createdBook.publisher);
    }
    
    async findById(id: string): Promise<Book | null> {
        const book = await this.prisma.book.findUnique({ where: { id } });
        if (!book) return null;
        return new Book(book.id, book.isbn, book.name, book.author, book.publisher);
    }
    ...
    async findAll(): Promise<Book[]> {
        ...
        return books.map(book => new Book(book.id, book.isbn, book.name, book.author, book.publisher));
    }
    ...
    async update(id: string, data: UpdateBookProps): Promise<Book> {
        ...
        return new Book(updatedBook.id, updatedBook.isbn, updatedBook.name, updatedBook.author, updatedBook.publisher);
    }
